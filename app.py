...
====================================================
Parametric & Non-parametric tests with automatic selection via Shapiro-Wilk normality test.
Tests: One-Sample T, Paired-Sample T, Independent-Sample T
       + Wilcoxon Signed-Rank (with Z), Mann-Whitney U (SPSS-exact)

SPSS Formula Fixes Applied:
  1. Levene's test uses center='mean' (SPSS default)
  2. Wilcoxon Z = (W - E[W]) / sqrt(Var[W] - ties_correction)  [SPSS exact]
  3. Mann-Whitney: U = min(U1,U2), W = rank_sum(group1),
     Mean Rank & Sum of Ranks displayed, Z computed from normal approximation
  4. Paired Samples Correlations table shown on both parametric tab and file download
"""
...
def wilcoxon_spss(diff_arr):
    """
    Compute Wilcoxon signed-rank test exactly as SPSS does.
    Returns: W (= min of pos/neg rank sums), Z, p, pos_rank_sum, neg_rank_sum,
             n_pos, n_neg, n_ties, n_total
    """
    diff_nz = diff_arr[diff_arr != 0]
    n = len(diff_nz)
    n_ties_pairs = len(diff_arr) - n

    if n < 1:
        return dict(W=np.nan, Z=np.nan, p=np.nan,
                    pos_rank_sum=0.0, neg_rank_sum=0.0,
                    n_pos=0, n_neg=0, n_ties=n_ties_pairs, n_total=len(diff_arr))

    abs_d = np.abs(diff_nz)
    ranks = stats.rankdata(abs_d)
    pos_rank_sum = float(np.sum(ranks[diff_nz > 0]))
    neg_rank_sum = float(np.sum(ranks[diff_nz < 0]))
    n_pos = int(np.sum(diff_nz > 0))
    n_neg = int(np.sum(diff_nz < 0))

    W = min(pos_rank_sum, neg_rank_sum)

    # Ties correction on variance
    ...
    return dict(W=W, Z=Z, p=p,
                pos_rank_sum=pos_rank_sum, neg_rank_sum=neg_rank_sum,
                n_pos=n_pos, n_neg=n_neg,
                n_ties=n_ties_pairs, n_total=len(diff_arr))

# ── SPSS-exact Mann-Whitney U ──────────────────────────────────────────────────
def mannwhitney_spss(g1, g2, label1, label2):
    """
    Compute Mann-Whitney U exactly as SPSS does.
    SPSS reports:
      - Mean Rank and Sum of Ranks per group
      - U = min(U1, U2)
      - Wilcoxon W = rank sum of the FIRST group listed
      - Z from normal approximation (with ties correction)
      - Asymptotic 2-tailed p from Z
    """
    ...
    all_ranks = stats.rankdata(all_data)      # handles ties via average ranks
    R1 = float(np.sum(all_ranks[:n1]))
    R2 = float(np.sum(all_ranks[n1:]))
    ...
    U  = min(U1, U2)
    W_wilcoxon = R1   # SPSS Wilcoxon W = rank sum of first listed group

    # Z with ties correction
    tie_groups = Counter(all_ranks)
    ties_corr  = sum(t**3 - t for t in tie_groups.values()) / (N*(N-1)) if N > 1 else 0
    ...
    return dict(
        ...
        R1=R1, R2=R2,
        mean_rank1=mean_rank1, mean_rank2=mean_rank2,
        n1=n1, n2=n2,
        Z=Z, p=p,
...
def test_normality(data, label=""):
    ...
    W, p = stats.shapiro(np.array(data))
...
def run_paired(data1, data2, label1="Var1", label2="Var2", alpha=0.05):
    ...
    # FIX 4: Pearson correlation (SPSS Paired Samples Correlations table)
    ...
    norm = test_normality(diff, f"{label1} − {label2} (differences)")
    R["normality"] = [norm]
    R["use_param"] = norm["pass"]
    ...
    # FIX 2: Wilcoxon SPSS-exact with Z
...
def run_independent(g1, g2, label1="Group 1", label2="Group 2",
    ...
    # FIX 1: Levene center='mean' (identical to SPSS)
    ...
    R["parametric"] = {
        ...
        "se_eq": se_eq,
        "ci_eq_l": mean_diff-tc_eq*se_eq, "ci_eq_u": mean_diff+tc_eq*se_eq,
    ...
    # FIX 3: Mann-Whitney SPSS-exact (U, W, Mean Rank, Sum of Ranks, Z)
...
def interpret_one_sample(R, var_name, alpha):
    ...
    pr   = R["parametric"]
    lines.append(
        f"<b>Normality (Shapiro-Wilk):</b> W = {_f(norm['W'])}, "
        f"p = {_p(norm['p'])}. Data "
        f"{'were approximately normally distributed → parametric test applied.' if norm['pass'] else 'deviated from normality → non-parametric test applied.'}"
    )
    if R["use_param"]:
        sig = pr["p_two"] < alpha
        lines.append(
            ...
            f"Mean difference = {_f(pr['mean_diff'])}, 95% CI [{_f(pr['ci_lower'])}, {_f(pr['ci_upper'])}], "
            f"Cohen's d = {_f(pr['cohens_d'])} ({effect_label_d(pr['cohens_d'])}). "
            f"The mean {'significantly differs from' if sig else 'does not significantly differ from'} μ₀ = {pr['mu0']}."
        )
    else:
        ...
        lines.append(
            ...
            f"Effect size r = {_f(np_r.get('r_eff',np.nan))}. "
...
def interpret_paired(R, alpha):
    ...
    lines.append(
        f"<b>Normality of Differences (Shapiro-Wilk):</b> W = {_f(norm['W'])}, "
        f"p = {_p(norm['p'])}. Differences "
        f"{'were normally distributed → parametric.' if norm['pass'] else 'were not normally distributed → non-parametric.'}"
    )
    lines.append(
    ...
    if R["use_param"]:
        ...
        lines.append(
            f"<b>Wilcoxon Signed-Rank Test:</b> W = {_f(np_r['W'],0)}, "
            f"Z = {_f(np_r['Z'])}, p = {_p(np_r['p'])} (2-tailed). "
            f"{'Significant' if sig else 'No significant'} difference between {pr['label1']} and {pr['label2']}."
        )
    return lines
...
def interpret_independent(R, dep_var, alpha):
    ...
    lev   = R["levene"]
    pr    = R["parametric"]
    norm_txt = "; ".join(f"{n['label']}: W = {_f(n['W'])}, p = {_p(n['p'])}" for n in norms)
    lines.append(
        f"<b>Normality (Shapiro-Wilk):</b> {norm_txt}. "
        f"{'Both groups normal → parametric.' if R['use_param'] else 'Non-normality detected → non-parametric.'}"
    )
    lines.append(
    ...
    if R["use_param"]:
        ...
        t_v  = pr["t_eq"]    if use_eq else pr["t_welch"]
        p_v  = pr["p_eq"]    if use_eq else pr["p_welch"]
        df_v = pr["df_eq"]   if use_eq else pr["df_welch"]
        ci_l = pr["ci_eq_l"] if use_eq else pr["ci_welch_l"]
        ci_u = pr["ci_eq_u"] if use_eq else pr["ci_welch_u"]
        sig  = p_v < alpha
        lines.append(
            f"<b>Independent T-Test ({'equal var.' if use_eq else 'Welch'}):</b> "
            f"t({_f(df_v,2)}) = {_f(t_v)}, p = {_p(p_v)} (2-tailed). "
            f"Mean diff = {_f(pr['mean_diff'])}, 95% CI [{_f(ci_l)}, {_f(ci_u)}], "
            f"Cohen's d = {_f(pr['cohens_d'])} ({effect_label_d(pr['cohens_d'])}). "
        ...
        sig  = np_r["p"] < alpha if not np.isnan(np_r["p"]) else False
        lines.append(
            f"<b>Mann-Whitney U Test:</b> U = {_f(np_r['U'],0)}, "
            f"W = {_f(np_r['W_wilcoxon'],0)}, Z = {_f(np_r['Z'])}, "
...
def plot_one_sample(data, mu0, var_name):
    ...
    ax1.plot(xr, stats.norm.pdf(xr,data.mean(),data.std(ddof=1))*sc, color=PAL[0], lw=2)
...
def plot_paired(data1, data2, label1, label2):
    ...
    for v1,v2 in zip(data1,data2):
        ax1.plot([0,1],[v1,v2],"-o",color=PAL[0] if v2>=v1 else PAL[1],alpha=0.45,markersize=4)
    ax1.plot([0,1],[data1.mean(),data2.mean()],"-o",color=PAL[1],lw=3,markersize=8)
    ...
    ax2.axvline(diff.mean(),color=PAL[2],lw=1.8,label=f"Mean={diff.mean():.2f}")
...
def plot_independent(g1, g2, label1, label2, dep_var):
    ...
    ax2 = fig.add_subplot(gs[1]); ax2.set_facecolor("#f8fafc")
    bins = np.linspace(min(g1.min(),g2.min())-.5,max(g1.max(),g2.max())+.5,15)
...
def html_tbl(rows, left_cols=None):
    """
    rows: list of lists. rows[0] = header.
    left_cols: set of column indices (0-based) that should be left-aligned.
    """
...
def build_pdf(test_type, R, meta, interps, fig_bytes_list):
    ...
    story.append(Paragraph("SPSS-Equivalent · Parametric & Non-Parametric",SUB))
    story.append(HRFlowable(width="100%",thickness=2,color=colors.HexColor("#e94560")))
    story.append(Spacer(1,6))

    mt = Table([[k,str(v)] for k,v in meta.items()],colWidths=[5*cm,11*cm])
    ...
    # 1. Normality
    story.append(Paragraph("  1. NORMALITY — SHAPIRO-WILK TEST",H1))
    nrows = [["Variable","N","W","Sig.","Result"]]
    for n in R["normality"]:
        nrows.append([n["label"],str(n["n"]),_f(n["W"]),_p(n["p"]),
                      "Normal" if n["pass"] else "Non-Normal"])
    story.append(mktbl(nrows))
    story.append(Paragraph(
        f"Note. {'Parametric' if R['use_param'] else 'Non-parametric'} analysis selected.",NT))
    ...
    if "correlation" in R:
        ...
        story.append(Paragraph("Note. Pearson correlation between paired variables.",NT))

    # Levene (independent)
    if "levene" in R:
        story.append(Paragraph(f"  {sec}. LEVENE'S TEST (center = mean)",H1)); sec+=1
        lev = R["levene"]
        story.append(mktbl([
    ...
    if test_type == "Independent-Sample T-Test":
        ...
        story.append(mktbl([
            ...
            [np_r["label1"],str(np_r["n1"]),_f(np_r["mean_rank1"]),_f(np_r["R1"],1)],
            [np_r["label2"],str(np_r["n2"]),_f(np_r["mean_rank2"]),_f(np_r["R2"],1)],
    ...
    story.append(Paragraph(
        ...
        "Levene center=mean · Wilcoxon Z with ties correction · "
        "Mann-Whitney SPSS exact",NT))
...
def main():
    ...
    <div class="main-hdr">
      ...
      <p>Parametric & Non-Parametric · Auto-selection via Shapiro-Wilk ·
         One-Sample · Paired-Sample · Independent-Sample</p>
        ...
        if df is not None:
            ...
            if test_type == "One-Sample T-Test":
                ...
                v2 = st.selectbox("Variable 2 (Post)", num_cols,
                                   index=num_cols.index("post_score") if "post_score" in num_cols else min(1,len(num_cols)-1))
                cfg = {"v1": v1, "v2": v2}
                ...
                if len(groups) >= 2:
                    ...
                    g2l = st.selectbox("Group 2", groups, index=min(1,len(groups)-1))
    ...
    if not run_btn and "stats_R" not in st.session_state:
        ...
        <div style="background:linear-gradient(135deg,#f0f9ff,#e0f2fe);
          ...
          <h4 style="margin:0 0 .4rem 0;font-size:.95rem;font-weight:700;">📋 {test_type}</h4>
        ...
        with st.spinner("Running analysis…"):
            try:
                if test_type == "One-Sample T-Test":
                    ...
                    meta = {"Test": test_type, "Variable 1": cfg["v1"],
                            "Variable 2": cfg["v2"], "N pairs": len(d1),
                            "α": alpha, "Date": datetime.now().strftime("%B %d, %Y %H:%M")}
                    interps = interpret_paired(R, alpha)
                    fig_main = plot_paired(d1, d2, cfg["v1"], cfg["v2"])

                else:
                    g1d = df[df[cfg["grp_col"]]==cfg["g1"]][cfg["dep_col"]].dropna().values.tolist()
                    g2d = df[df[cfg["grp_col"]]==cfg["g2"]][cfg["dep_col"]].dropna().values.tolist()
                    R   = run_independent(g1d, g2d, cfg["g1"], cfg["g2"], cfg["dep_col"], alpha)
                    meta = {"Test": test_type,
                    ...
                    fig_main = plot_independent(g1d, g2d, cfg["g1"], cfg["g2"], cfg["dep_col"])

            except Exception as e:
                st.error(f"Analysis error: {e}"); import traceback; st.code(traceback.format_exc()); return
    ...
    st.markdown(
        ...
        f'{"✅" if use_p else "⚠️"} '
        f'Shapiro-Wilk: p {">" if use_p else "≤"} .05 → '
    ...
    _ti_0 = ti; ti += 1
    with tabs[_ti_0]:
        st.markdown('<div class="sec-title">Tests of Normality — Shapiro-Wilk</div>',
                    unsafe_allow_html=True)
        rows = [["Variable","N","Statistic (W)","Sig.","Result"]]
        for n in R["normality"]:
            res = ('<span class="pass">✓ Normal (p > .05)</span>' if n["pass"]
                   else '<span class="fail">✗ Non-Normal (p ≤ .05)</span>')
            rows.append([n["label"],str(n["n"]),_f(n["W"]),_p(n["p"]),res])
        st.markdown(html_tbl(rows,left_cols={0,4}), unsafe_allow_html=True)
        st.markdown('<p class="note-txt">p > .05 → parametric test applied. '
                    'p ≤ .05 → non-parametric test applied.</p>', unsafe_allow_html=True)
        if use_p:
            st.markdown('<div class="info-box">✅ <b>Normality met.</b> Parametric analysis applied.</div>',
                        unsafe_allow_html=True)
        else:
            st.markdown('<div class="warn-box">⚠️ <b>Normality violated.</b> Non-parametric analysis applied.</div>',
                        unsafe_allow_html=True)
    ...
    # ── Tab: Paired Correlation (FIX 4) ──────────────────────────────────────
    if "correlation" in R:
        _ti_2 = ti; ti += 1
        with tabs[_ti_2]:
            st.markdown('<div class="sec-title">Paired Samples Correlations</div>',
                        unsafe_allow_html=True)
            corr = R["correlation"].copy()
            corr["Pearson Correlation"] = corr["Pearson Correlation"].apply(_f)
            corr["Sig. (2-tailed)"]     = corr["Sig. (2-tailed)"].apply(_p)
            st.markdown(html_tbl(df_to_rows(corr),left_cols={0}), unsafe_allow_html=True)
            st.markdown('<p class="note-txt">Pearson correlation between the two paired variables.'
                        ' Significant correlation supports the paired design.</p>',
                        unsafe_allow_html=True)

    # ── Tab: Levene's test (FIX 1) ────────────────────────────────────────────
    if "levene" in R:
        _ti_3 = ti; ti += 1
        with tabs[_ti_3]:
            lev = R["levene"]
            res = ('<span class="pass">✓ Equal variances assumed</span>' if lev["equal_var"]
                   else '<span class="fail">✗ Equal variances NOT assumed (use Welch)</span>')
            st.markdown('<div class="sec-title">Levene\'s Test for Equality of Variances '
                        '(center = mean, SPSS default)</div>', unsafe_allow_html=True)
            st.markdown(html_tbl(
                [["F","df1","df2","Sig.","Result"],
                 [_f(lev["F"]),str(lev["df1"]),str(lev["df2"]),_p(lev["Sig."]),res]],
                left_cols={4}), unsafe_allow_html=True)
            st.markdown('<p class="note-txt">p > .05 → equal variances assumed → use row 1 of t-test table.<br>'
                        'p ≤ .05 → equal variances not assumed → use Welch row (row 2).</p>',
                        unsafe_allow_html=True)
    ...
    with tabs[_ti_4]:
        if test_type == "One-Sample T-Test":
            st.markdown(f'<div class="sec-title">One-Sample Test · Test Value (μ₀) = {pr["mu0"]}</div>',
                        unsafe_allow_html=True)
            st.markdown(html_tbl([
                ["","t","df","Sig. (2-tailed)","Sig. (1-tailed Lower)","Sig. (1-tailed Upper)",
            ...
            # FIX 4: Show correlation table here too
            st.markdown('<div class="sec-title">Paired Samples Correlations</div>',
                        unsafe_allow_html=True)
            corr = R["correlation"].copy()
            corr["Pearson Correlation"] = corr["Pearson Correlation"].apply(_f)
            ...
            st.markdown('<div class="sec-title">Paired Samples Test</div>',
                        unsafe_allow_html=True)
            st.markdown(html_tbl([
            ...
            st.markdown('<div class="sec-title">Independent Samples Test</div>',
                        unsafe_allow_html=True)
            lev = R["levene"]
            st.markdown(html_tbl([
            ...
            st.markdown(f'<p class="note-txt">Based on Levene p = {_p(lev["Sig."])}: '
                        f'use <b>{active}</b>.</p>', unsafe_allow_html=True)

        if not use_p:
            st.markdown('<div class="warn-box">⚠️ Normality violated — '
                        'see Non-Parametric tab for recommended results.</div>',
                        unsafe_allow_html=True)
    ...
    with tabs[_ti_5]:
        if test_type == "Independent-Sample T-Test":
            # FIX 3: Mann-Whitney with Ranks + W + Z
            st.markdown('<div class="sec-title">Mann-Whitney U Test — Ranks</div>',
                        unsafe_allow_html=True)
            st.markdown(html_tbl([
                ["Group","N","Mean Rank","Sum of Ranks"],
            ...
            st.markdown('<div class="sec-title">Mann-Whitney U Test — Test Statistics</div>',
                        unsafe_allow_html=True)
            st.markdown(html_tbl([
                ["Statistic","Value"],
                ["Mann-Whitney U",      _f(np_r["U"],0)],
            ...
            st.markdown('<p class="note-txt">Grouping variable: '
                        f'{np_r["label1"]} vs. {np_r["label2"]}. '
                        'Z based on normal approximation with ties correction.</p>',
                        unsafe_allow_html=True)

        else:
            # FIX 2: Wilcoxon with Z
            n_neg  = np_r.get("n_neg", 0)
            n_pos  = np_r.get("n_pos", 0)
            n_ties = np_r.get("n_ties", 0)
            n_tot  = np_r.get("n_total", n_neg+n_pos+n_ties)
            ...
            st.markdown('<div class="sec-title">Wilcoxon Signed-Rank Test — Ranks</div>',
                        unsafe_allow_html=True)
            footnotes = []
            if test_type == "Paired-Sample T-Test":
                footnotes = [
                    f"a. {pr['label2']} < {pr['label1']}",
                    f"b. {pr['label2']} > {pr['label1']}",
                    f"c. {pr['label2']} = {pr['label1']}"
                ]
            st.markdown(html_tbl([
                ["","N","Mean Rank","Sum of Ranks"],
            ...
            for fn in footnotes:
                st.markdown(f'<p class="note-txt">{fn}</p>', unsafe_allow_html=True)

            st.markdown('<div class="sec-title">Wilcoxon Signed-Rank Test — Test Statistics</div>',
                        unsafe_allow_html=True)
            ...
            st.markdown('<p class="note-txt">Based on '
                        f'{"negative" if n_neg < n_pos else "positive"} ranks. '
                        'Z computed using ties-corrected variance (SPSS method).</p>',
                        unsafe_allow_html=True)

        if use_p:
            st.markdown('<div class="info-box">ℹ️ Normality was met — '
    ...
    with tabs[_ti_7]:
        ...
        for line in interps:
            cls = ""
            if "significant difference" in line.lower() and "no statistically" not in line.lower() and "not significant" not in line.lower():
                cls = "sig"
            elif "no significant" in line.lower() or "not significant" in line.lower():
                cls = "nonsig"
        ...
        if use_p:
            if test_type == "One-Sample T-Test":
                ...
                apa  = (f"A one-sample t-test was conducted to examine whether "
                        f"{meta.get('Variable','the variable')} (M = {_f(m_v)}, SD = {_f(sd_v)}) "
                        f"differed from μ₀ = {pr['mu0']}. "
                        f"The test was {'statistically significant' if pr['p_two']<alpha else 'not significant'}, "
                        f"t({pr['df']}) = {pr['t']:.2f}, "
                        f"p {'< .001' if pr['p_two']<.001 else '= '+_p(pr['p_two'])}, "
                        f"d = {pr['cohens_d']:.2f}.")
            elif test_type == "Paired-Sample T-Test":
                apa = (f"A paired-samples t-test compared {pr['label1']} "
                       ...
                       f"The difference was {'significant' if pr['p_two']<alpha else 'not significant'}, "
                       f"t({pr['df']}) = {pr['t']:.2f}, "
                       f"p {'< .001' if pr['p_two']<.001 else '= '+_p(pr['p_two'])}, "
                       f"d = {pr['cohens_d']:.2f}.")
            else:
                ...
                apa = (f"An independent-samples t-test compared {pr['dep_var']} "
                       f"between {pr['label1']} "
                       f"(M = {_f(float(R['desc'].iloc[0]['Mean']))}, "
                       f"SD = {_f(float(R['desc'].iloc[0]['Std. Deviation']))}) "
                       f"and {pr['label2']} "
                       f"(M = {_f(float(R['desc'].iloc[1]['Mean']))}, "
                       f"SD = {_f(float(R['desc'].iloc[1]['Std. Deviation']))}). "
                       f"The difference was {'significant' if pv<alpha else 'not significant'}, "
                       f"t({_f(dv,2)}) = {tv:.2f}, "
                       f"p {'< .001' if pv<.001 else '= '+_p(pv)}, "
                       f"d = {pr['cohens_d']:.2f}.")
        else:
            if test_type == "Independent-Sample T-Test":
                apa = (f"A Mann-Whitney U test compared {np_r['dep_var']} between "
                       f"{np_r['label1']} and {np_r['label2']}, "
                       f"U = {np_r['U']:.0f}, W = {np_r['W_wilcoxon']:.1f}, "
                       f"Z = {np_r['Z']:.3f}, "
                       f"p {'< .001' if np_r['p']<.001 else '= '+_p(np_r['p'])} (asymptotic, 2-tailed).")
            else:
                apa = (f"A Wilcoxon signed-rank test was conducted: "
                       f"W = {np_r['W']:.0f}, Z = {np_r['Z']:.3f}, "
                       f"p {'< .001' if np_r['p']<.001 else '= '+_p(np_r['p'])} (2-tailed).")
    ...
    with dc2:
        ...
        with pd.ExcelWriter(xbuf, engine="openpyxl") as writer:
            ...
            pd.DataFrame(R["normality"]).to_excel(writer, sheet_name="Shapiro-Wilk", index=False)
            if "correlation" in R:
